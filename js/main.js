
var curIndex = 0;
//时间间隔(单位毫秒)，每秒钟显示一张，数组共有3张图片放在img文件夹下
var timeInterval = 500;

                        //定义一个存放照片位置的数组，可以放任意个，在这里放3个
var arr = new Array();
  arr[0] = "technology.jpg";
  arr[1] = "technology2.jpg";
  arr[2] = "technology3.jpg";
  setInterval(changeImg, timeInterval);
function changeImg() {
  if (curIndex == arr.length - 1) {
    curIndex = 0;
  } else {
    curIndex += 1;
  }
                                  //设置body的背景图片
  document.getElementById('banner').style.backgroundImage = "URL("+arr[curIndex]+" )";
  document.getElementById('banner').style.backgroundSize = 'cover';
  document.getElementById('banner').style.backgroundAttachment = 'fixed';
  document.getElementById('banner').style.height = '400px';
  document.getElementById('banner').style.padding = '150px';
  document.getElementById('banner').style.position = 'relative';
  document.getElementById('banner').style.overflow = 'hidden';
}
